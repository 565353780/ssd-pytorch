...
set my_train_mode in ./data/voc7012.py and ./test.py to True
run ./test.py
"""
...
import numpy as np
import argparse
from data.voc0712 import my_train_mode
...
# parser.add_argument('--basenet', default=resume_name.split('weights/')[1],
#                     help='Pretrained base model')
parser.add_argument('--batch_size', default=16, type=int,
                    help='Batch size for training')
parser.add_argument('--resume', default=resume_name, type=str,
                        help='Checkpoint state_dict file to resume training from')
parser.add_argument('--start_iter', default=(int(resume_num)+1), type=int,
...
def train():
    if args.dataset == 'COCO':
        ...
        cfg = voc
        if my_train_mode:
            dataset = VOCDetection(root=args.dataset_root, image_sets=[(dataname, 'trainval')],
                                   transform=SSDAugmentation(cfg['min_dim'],
                                                             MEANS))
        else:
